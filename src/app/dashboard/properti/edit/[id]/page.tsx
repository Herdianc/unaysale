...
export default function EditPropertiPage() {
  ...
  useEffect(() => {
    async function loadData() {
      setFetching(true);
      try {
        const [catRes, distRes, facRes, propRes] = await Promise.all([
        ...
        if (!propRes.ok) {
          setFetchError("Properti tidak ditemukan");
          return;
        }

        const prop: PropertyData = await propRes.json();
        setForm({
          title: prop.title || "",
          description: prop.description || "",
          transactionType: prop.transactionType || "DIJUAL",
          ...
          villageId: prop.villageId || "",
          latitude: prop.latitude ? String(prop.latitude) : "",
          longitude: prop.longitude ? String(prop.longitude) : "",
          landArea: prop.landArea ? String(prop.landArea) : "",
          buildingArea: prop.buildingArea ? String(prop.buildingArea) : "",
          ...
          carport: prop.carport ? String(prop.carport) : "",
          garage: prop.garage ? String(prop.garage) : "",
          certificate: prop.certificate || "",
          pbb: prop.pbb || false,
          imb: prop.imb || false,
          facilityIds: prop.facilityIds || [],
          imageUrls: prop.imageUrls || [],
        });
      } catch (e) {
        console.error("Failed to load data:", e);
        setFetchError("Gagal memuat data properti");
      } finally {
        setFetching(false);
      }
    }
  ...
  }, [form.districtId, districts, form.villageId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        garage: form.garage ? parseInt(form.garage, 10) : undefined,
      ...
      if (!res.ok) {
        ...
        setError(data?.message || "Gagal menyimpan properti");
        return;
      }
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-sm text-gray-500">Memuat data properti...</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
            ...
            <Button variant="outline" onClick={() => router.push("/dashboard/properti")}>
          ...
          {currentStep === 2 && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Garasi (mobil)</label>
                  <Input
                    type="number"
                    placeholder="0"
                    value={form.garage}
                    onChange={(e) => updateForm({ garage: e.target.value })}
                  />
                </div>
              </div>
            </div>
          ...
          {currentStep === 5 && (
            <div className="space-y-4">
              <p className="text-sm text-gray-500">
                Masukkan URL foto properti. Foto pertama akan menjadi foto utama.
              </p>
